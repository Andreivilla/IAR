...
models = [build_model(len(X.columns), args.hidden_layers, learning_rate, f'{learning_rate:.3f}') for learning_rate in args.learning_rates]
model_names = [model.name for model in models]

# 6) Treinar o modelo
histories = [model.fit(X_train, y_train, epochs=50,
        batch_size=2000,
        validation_split=0.2) for model in models]
...
multiplier = 0
for name, values in [('Acurácia', accuracies), ('Entropia cruzada', losses)]:
    offset = width * multiplier
    rects = bars_ax.bar(x + offset, [float(f'{x:.4f}') for x in values], width, label=name)
    bars_ax.bar_label(rects, padding=3)
    multiplier += 1
bars_ax.set_title('Acurácias e Perdas')
bars_ax.set_xticks(x + width, model_names)
bars_ax.set_ylim(0, 1.2)
...
plot_stats_graph([history.history['accuracy'] for history in histories], train_acc_ax, 'Acurácidade no treinamento', 'Acurácia')
plot_stats_graph([history.history['loss'] for history in histories], train_loss_ax, 'Perda no treinamento', 'loss')
plot_stats_graph([history.history['val_accuracy'] for history in histories], val_acc_ax, 'Acurácidade na validação', 'Acurácia')
plot_stats_graph([history.history['val_loss'] for history in histories], val_loss_ax, 'Perda na validação', 'loss')
plt.tight_layout()
...
# Suponha que model_names e y_preds_class sejam definidos anteriormente
...
for model_name, y_pred in zip(model_names, y_preds_class):
    ...
    save_or_show(f'{model_name}_confusion_matrix.png')
