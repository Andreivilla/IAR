...
class Enviroment:
    ...
    def ants_matrix(self):
        ...
        for ant in self.ants:
            #ant_position = ant.position #get_position()
            self.enviorment_ants[ant.position[0]][ant.position[1]] = ant.name
    ...
    def choose_random_zero(self):
        while True:
            ...
            if int(self.enviorment[i][j].name) == 0: #.get_name()) == 0:
    ...
    def add_new_ant(self, vison_degree):#vision_degree cria o nivel de visão da formiga
        if len(self.ants) == 0:
            ...
            new_ant = Ant(self.ants[-1].id+1, 'b', self.choose_random_zero(), vison_degree)    
    ...
    def mov_ants(self):
        for ant in self.ants:            
            ...
            ant.set_underiten(self.get_enviroment_value(ant.position))# get_position()))
            ant_vision_data = self.get_ant_vision(ant, 1) #o 1 poderia ser qualquer valor
            
            if int(ant.get_underiten().name) >= 1:#get_underiten().get_name()) >= 1:#ponto com iten decidir se formiga pega ou não o iten
                if ant.take_iten(ant_vision_data):
                    self.take_enviroment_iten(ant.position)#get_position())
            else:#ponto vazio decidir se a formiga deve largar algo
                iten_drop = ant.drop_iten(ant_vision_data)
                if iten_drop != False:
                    self.drop_enviroment_iten(ant.position, iten_drop)
